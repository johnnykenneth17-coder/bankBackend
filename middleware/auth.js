...
// Check if session is still valid (single device)
const checkSingleDeviceSession = async (req, res, next) => {
    try {
        ...
        // Check if this session ID matches the active session in users table
        const { data: user, error } = await supabase
            .from('users')
        ...
        // Check if there's an active session ID stored
        if (user.active_session_id && user.active_session_id !== decoded.sessionId) {
            // Session has been invalidated by another device
            await supabase
                .from('user_sessions')
                .update({ 
                    is_active: false, 
            ...
            return res.status(401).json({ 
                ...
                message: 'You have been logged out because a new login was detected on another device. If you did not authorize this, please log in again and change your credentials immediately.',
...
async function createUserSession(userId, token, deviceInfo, req) {
    try {
        const sessionId = generateSessionId();
        const expiresAt = new Date();
        expiresAt.setDate(expiresAt.getDate() + 7); // 7 days expiry
        
        // First, get the old active session ID to notify if needed
        ...
        // If there was a previous active session, create a notification for that user
        if (hadActiveSession) {
            await supabase
                .from('notifications')
                .insert({
...
async function getUserActiveSessions(userId) {
    const { data: sessions, error } = await supabase
        .from('user_sessions')
        .select('id, device_fingerprint, ip_address, user_agent, created_at, last_activity, is_current')
        .eq('user_id', userId)
        .eq('is_active', true)

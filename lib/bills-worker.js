...
async function processBillPaymentJob(job) {
  ...
  if (!result.success) {
    if (result.retryable) {
      await supabase.from("bill_payments").update({ status: "reserved" }).eq("id", bill_payment_id);
      ...
      if (rpcErr) {
        console.error("[BILLS-WORKER] finalize_bill_payment (failed) RPC failed:", rpcErr);
        await failOrRetryJob(job, `Also failed to release reservation: ${rpcErr.message}`);
        return;
      }
      await markJobCompleted(job.id);
  ...
  await pollAndFinalize({ ...bill, provider_reference: result.data.flw_ref }, providerCode, job);
}
...
async function pollAndFinalize(bill, providerCode, job) {
  ...
  if (statusCheck.data.confirmed) {
    ...
    if (rpcErr) {
      console.error("[BILLS-WORKER] finalize_bill_payment (completed) RPC failed:", rpcErr);
      await failOrRetryJob(job, `Confirmed by provider but failed to record completion: ${rpcErr.message}`);
      return;
    }
    await markJobCompleted(job.id);
    console.log(`[BILLS-WORKER] Job ${job.id} completed — bill payment ${bill.id} confirmed`);
    return;
  }
  ...
  await failOrRetryJob(job, `Provider has not confirmed bill ${bill.provider_tx_ref} yet, will re-check`);
}

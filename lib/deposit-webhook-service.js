...
async function handleFlutterwaveWebhook(req, res) {
  ...
  // those. Only `id` is common to both shapes.
  ...
  // dedup signal.
  //
  // IMPORTANT: the key is `${event}:${data.id}`, not just data.id.
  // Flutterwave's `id` is scoped per resource type — charge.completed
  // and transfer.* events draw from separate counters and CAN share the
  // same numeric id (especially in test mode, where the numbers are
  // small). Keying on data.id alone meant a transfer webhook could
  // collide with an unrelated, already-processed deposit webhook that
  // happened to have the same id, get treated as a duplicate, and be
  // ack'd with zero processing — which is exactly why transfers were
  // staying stuck on "pending" with no outgoing verification call ever
  // being made.
  const webhookKey = `${event}:${data.id}`;
  ...
  if (logErr) {
    ...
    if (logErr.code === "23505") {
      ...
      if (existing && existing.processed) {
        console.log(
          `[WEBHOOK] Duplicate ${webhookKey}, already processed — ack only`,
        );

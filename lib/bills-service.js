...
const billPaymentLimiter = rateLimit({
  ...
  legacyHeaders: false,
  message: { error: "Too many bill payment requests. Please try again shortly." },
...
function contextHash({ service_type, customer_identifier, amount }) {
  return crypto
    .createHash("sha256")
    .update(`${service_type}:${customer_identifier}:${Number(amount).toFixed(2)}`)
    .digest("hex");
}
...
async function getBillPaymentFeePercentage(serviceType) {
  const { data } = await supabase
    ...
    .eq("setting_key", `bill_payment_fee_percentage_${serviceType.toLowerCase()}`)
    .single();
...
async function handleVerifyBillPaymentPin(req, res) {
  ...
  if (!service_type || !customer_identifier || !amount) {
    return res.status(400).json({
      valid: false,
      error: "service_type, customer_identifier and amount are required to bind this PIN check",
    });
  }

  const validation = validateBillRequest({ service_type, customer_identifier, amount });
  if (!validation.valid) {
  ...
  try {
    if (!from_account_id || !service_type || !customer_identifier || !amount) {
      return res.status(400).json({
        ...
        code: "MISSING_FIELDS",
        required: ["from_account_id", "service_type", "customer_identifier", "amount"],
      });
    }
    if (!idempotency_key) {
      return res.status(400).json({
    ...
    const hash = contextHash({ service_type, customer_identifier, amount: numericAmount });
    ...
    const feeAmount = Math.round(numericAmount * (feePercentage / 100) * 100) / 100;

    const providerTxRef = `FEECENT-BILL-${crypto.randomUUID()}`;

    const { data: result, error: rpcErr } = await supabase.rpc("reserve_bill_payment", {

...
// Flutterwave's API (flutterwaveService.verifyTransferStatus) and only
...
async function processTransferEvent({ event, data, webhookLogId }) {
  ...
  const verification = await flutterwaveService.verifyTransferStatus(data.id);
...
async function reconcileStuckTransfers(limit = 20) {
  ...
  for (const row of stuck || []) {
    ...
    const verification = await flutterwaveService.verifyTransferStatus(
